...
export default function LotteryPanel() {
  ...
  const [mode, setMode] = useState<"random-no-save" | "round-robin">("round-robin");
  ...
  const currentClassNominations = nominations.filter(n => selectedClass?.items.includes(n.itemName));
  const isAllDrawn = selectedClass && currentClassNominations.length >= selectedClass.items.length;

  const handleDraw = async () => {
    if (!selectedClass || selectedClass.items.length === 0 || (mode === "round-robin" && isAllDrawn)) return;
    ...
    let count = 0;
    const shuffle = () => {
      setShuffleName(selectedClass.items[Math.floor(Math.random() * selectedClass.items.length)]);
      count++;
      if (count < 20) {
    ...
    setTimeout(async () => {
      ...
      if (mode === "random-no-save") {
        ...
        if (selected) {
          await lotteryDB.addNomination({ classId: selectedClass.id, itemName: selected });
    ...
    <div className="max-w-5xl mx-auto p-6 bg-slate-50 min-h-screen space-y-6">
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-wrap items-end gap-6">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-sm font-semibold text-slate-700 mb-2">グループ選択</label>
          <select
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl items-center h-[52px]">
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${
              mode === "random-no-save" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
            }`}
          >
            ランダム
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${
              mode === "round-robin" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
            }`}
          >
            一巡モード
          </button>
      ...
      `}>
        <div className={`flex-1 flex flex-col items-center justify-center p-8 space-y-6 ${isFullscreen ? "bg-white " : "text-slate-900"}`}>
            ...
            {(() => {
              ...
              return (
                <div className="space-y-4">
                  {/* item1: 上段 (小さめ) */}
                  <div className={`font-bold uppercase tracking-widest opacity-40 ${isFullscreen ? "text-6xl" : "text-4xl"} h-12`}>
                    {isDrawing ? "DRAWING..." : (item2 || selectedClass?.name || "STANDBY")}
                  </div>
                  
                  {/* item2: 中段 (メイン: 特大) */}
                  <div className={`font-black tracking-tight leading-none ${isFullscreen ? "text-[15rem]" : "text-8xl"} min-h-[1em]`}>
                    { item1 || (isDrawing ? "" : "READY?")}
                  </div>

                  {/* item3: 下段 (メッセージ等) */}
                  <div className={`font-bold text-indigo-600 flex items-center justify-center ${isFullscreen ? "text-4xl " : "text-xl"} h-10`}>
                    {!isDrawing && (item3 || (result ? "おめでとうございます！" : ""))}
                  </div>
                </div>
              );
            })()}
        ...
        <div className={`p-8 flex justify-center border-t border-slate-100 bg-slate-50`}>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <h3 className="text-sm font-bold text-slate-500 uppercase mb-4 border-b border-slate-50 pb-2">現在の進捗</h3>
          <div className="flex justify-between items-end mb-2">
            <span className="text-4xl font-bold text-slate-900">{currentClassNominations.length}<small className="text-slate-400 text-lg ml-1">/{selectedClass?.items.length || 0}</small></span>
            <span className="text-indigo-600 font-bold">{Math.round((currentClassNominations.length / (selectedClass?.items.length || 1)) * 100)}%</span>
          </div>
          <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
            <div className="bg-indigo-600 h-full transition-all duration-700" style={{ width: `${(currentClassNominations.length / (selectedClass?.items.length || 1)) * 100}%` }}></div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <h3 className="text-sm font-bold text-slate-800 uppercase">当選履歴</h3>
            <button onClick={handleClearHistory} disabled={nominations.length === 0} className="text-[10px] font-bold text-red-500 hover:bg-red-50 px-3 py-1 rounded-full transition-colors">履歴をクリア</button>
          </div>
          <div className="p-4 flex flex-wrap gap-2 overflow-y-auto max-h-[160px]">
            {nominations.length === 0 ? (
              <p className="w-full text-center py-8 text-slate-400 text-sm italic">履歴はありません</p>
            ) : (
              nominations.map((nom, i) => (
                <div key={nom.id} className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-2">
                  <span className="text-[10px] text-slate-400 font-bold">#{nominations.length - i}</span>
                  <span className="font-medium text-slate-700 text-sm">{nom.itemName}</span>
                </div>
              ))
            )}
          </div>
